...
const BlogPostClient: React.FC<BlogPostClientProps> = ({ post, blogStats }) => {
  const [currentStats, setCurrentStats] = React.useState({
    views: blogStats?.[post.id]?.views || post.views || 0,
    likes: blogStats?.[post.id]?.likes || post.likes || 0,
    isLiked: false
  });
  ...
  return (
    <main className="relative min-h-screen bg-white flex flex-col overflow-x-hidden">
      ...
      <div className="w-full max-w-full sm:max-w-2xl md:max-w-3xl lg:max-w-4xl xl:max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <Link
          href="/team-blog"
          className="inline-flex items-center text-[#BF608F] hover:text-[#a34e7a] mb-6 font-arsenal text-sm"
        >
          ← Back to Blog
        </Link>
        ...
        <header className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              ...
              <span className="font-arsenal">{post.comments || 0} comments</span>
            </div>
          </div>
